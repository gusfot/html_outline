;(function () {
	...
	var offCanvass = function() {
		$('body').on('click', '.js-wkk-nav-toggle', function(event){

			var $this = $(this);

			$('#wkk-offcanvass').toggleClass('wkk-awake');
			$('#wkk-page, #wkk-menu').toggleClass('wkk-sleep');




			if ( $('#wkk-offcanvass').hasClass('wkk-awake') ) {
	...
	var mobileMenuOutsideClick = function() {
	    ...
	    var container = $("#wkk-offcanvass, .js-wkk-nav-toggle");
	    if (!container.is(e.target) && container.has(e.target).length === 0) {
	    	if ( $('#wkk-offcanvass').hasClass('wkk-awake') ) {
	    		$('#wkk-offcanvass').removeClass('wkk-awake');
	    		$('#wkk-page, #wkk-menu').removeClass('wkk-sleep');

	    		$('.js-wkk-nav-toggle').removeClass('active');
	    	}
	    }
		});
		...
		$(window).scroll(function(){
			...
			if ( $(window).scrollTop() < 500 ) {
				...
				$('#wkk-offcanvass ul li').removeClass('active');
				$('#wkk-offcanvass ul li').first().addClass('active');
			}
	...
	var introWayPoint = function() {
		if ( $('#wkk-hero').length > 0 ) {
			$('#wkk-hero').waypoint( function( direction ) {
	...
	// Pengki Animate	
	var pengkiAnimate = function() {
		if ( $('#wkk-pengki').length > 0 ) {	
			$('#wkk-pengki .to-animate').each(function( k ) {
	...
	var pengkiWayPoint = function() {
		if ( $('#wkk-pengki').length > 0 ) {
			$('#wkk-pengki').waypoint( function( direction ) {
										
				if( direction === 'down' && !$(this).hasClass('animated') ) {
					
				
					setTimeout(pengkiAnimate, 100);
	...
	var clientAnimate = function() {
		if ( $('#wkk-clients').length > 0 ) {	
			$('#wkk-clients .to-animate').each(function( k ) {
	...
	var clientWayPoint = function() {
		if ( $('#wkk-clients').length > 0 ) {
			$('#wkk-clients').waypoint( function( direction ) {
	...
	var featuresAnimate = function() {
		if ( $('#wkk-features').length > 0 ) {	
			$('#wkk-features .to-animate').each(function( k ) {
	...
	var featuresWayPoint = function() {
		if ( $('#wkk-features').length > 0 ) {
			$('#wkk-features').waypoint( function( direction ) {
	...
	var features2AnimateTitle = function() {
		if ( $('#wkk-features-2').length > 0 ) {	
			$('#wkk-features-2 .to-animate').each(function( k ) {
	...
	var features2WayPoint = function() {
		if ( $('#wkk-features-2').length > 0 ) {
			$('#wkk-features-2').waypoint( function( direction ) {
	...
	var counterWayPoint = function() {
		if ( $('#wkk-counter').length > 0 ) {
			$('#wkk-counter').waypoint( function( direction ) {
	...
	var productsAnimate = function() {
		if ( $('#wkk-products').length > 0 ) {	
			$('#wkk-products .to-animate').each(function( k ) {
	...
	var productsWayPoint = function() {
		if ( $('#wkk-products').length > 0 ) {
			$('#wkk-products').waypoint( function( direction ) {
	...
	var ctaAnimate = function() {
		if ( $('#wkk-cta').length > 0 ) {	
			$('#wkk-cta .to-animate').each(function( k ) {
	...
	var ctaWayPoint = function() {
		if ( $('#wkk-cta').length > 0 ) {
			$('#wkk-cta').waypoint( function( direction ) {
	...
	var pricingAnimate = function() {
		if ( $('#wkk-pricing').length > 0 ) {	
			$('#wkk-pricing .to-animate').each(function( k ) {
	...
	var pricingWayPoint = function() {
		if ( $('#wkk-pricing').length > 0 ) {
			$('#wkk-pricing').waypoint( function( direction ) {
	...
	var features3Animate = function() {
		if ( $('#wkk-features-3').length > 0 ) {	
			$('#wkk-features-3 .to-animate').each(function( k ) {
	...
	var features3WayPoint = function() {
		if ( $('#wkk-features-3').length > 0 ) {
			$('#wkk-features-3').waypoint( function( direction ) {
	...
	var faqsAnimate = function() {
		if ( $('#wkk-faqs').length > 0 ) {	
			$('#wkk-faqs .to-animate').each(function( k ) {
	...
	var faqsWayPoint = function() {
		if ( $('#wkk-faqs').length > 0 ) {
			$('#wkk-faqs').waypoint( function( direction ) {
